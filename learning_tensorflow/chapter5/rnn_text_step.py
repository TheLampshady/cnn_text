...
for i in range(10000):
    ...
    even_sentences.append(" ".join(
        [digit_to_word_map[r] for r in rand_odd_ints]
    ))
    odd_sentences.append(" ".join(
        [digit_to_word_map[r] for r in rand_even_ints]
    ))

data = even_sentences + odd_sentences  # Same seq lengths for even, odd sentences seqlens* = 2
...
def get_sentence_batch(batch_size, data_x, data_y, data_seqlens):
    ...
    x = [
        [
            word2index_map[word]
            for word in data_x[i].lower().split()
        ] for i in batch
    ]
...
with tf.Session() as sess:
    ...
    output_example, states_example = sess.run(
        [outputs, states],
        feed_dict={_inputs: x_test, _labels: y_test, _seqlens: seqlen_test}
    )
